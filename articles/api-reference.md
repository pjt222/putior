...
This vignette provides comprehensive documentation for all exported
functions in the putior package.
...
The primary function for extracting PUT annotations from source code
files.
...
Analyzes source code to automatically detect inputs, outputs, and
dependencies without requiring manual annotations.
...
Analyzes code and generates PUT annotation comments that can be added to
source files. Similar to how roxygen2 generates documentation skeletons.
...
Combines manually written annotations with auto-detected ones using
...
Returns the detection patterns used for auto-detecting inputs, outputs,
and dependencies.
...
Sets the logging verbosity for putior functions. Requires the optional
`logger` package.
...
Launches an interactive Shiny app for experimenting with PUT
annotations.
