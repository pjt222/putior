...
multilang_ml_workflow <- data.frame(
  ...
  id = c("load", "eda", "features", "split", "train", "hyper", "final", "evaluate", "report", "deploy_export"),
...
```{r before-diagram, echo=FALSE, results='asis', eval=TRUE}
library(putior)
before_workflow <- data.frame(
  file_name = rep("etl_pipeline.R", 3),
  id = c("step1", "2", "final_step_1"),
  label = c("step1", "2", "final step"),
...
cat("```mermaid\n")
cat(put_diagram(before_workflow, theme = "github", output = "raw"))
cat("\n```\n")
```
...
```{r after-diagram, echo=FALSE, results='asis', eval=TRUE}
after_workflow <- data.frame(
  file_name = rep("etl_pipeline.R", 4),
  id = c("extract_sales", "clean_data", "aggregate_regions", "export_results"),
  label = c("Load Sales Data", "Clean & Validate", "Aggregate by Region", "Export Regional Report"),
  node_type = c("input", "process", "process", "output"),
  input = c(NA, "sales_2024.csv", "clean_sales.internal", "aggregated.internal"),
  output = c("sales_2024.csv", "clean_sales.internal", "aggregated.internal", "regional_sales.csv"),
  stringsAsFactors = FALSE
)
cat("```mermaid\n")
cat(put_diagram(after_workflow, theme = "github", output = "raw"))
cat("\n```\n")
```
...
|----------|---------|---------------|
| `put()` | Extract annotations | [API Reference](api-reference.html#put) |
| `put_diagram()` | Generate diagrams | [API Reference](api-reference.html#put_diagram) |
| `put_auto()` | Auto-detect workflows | [Features Tour](features-tour.html#put_auto---detect-workflow-automatically) |
| `put_merge()` | Combine manual + auto | [Features Tour](features-tour.html#put_merge---combine-manual--auto) |

**Related guides:**

- [Quick Start](quick-start.html) - Create your first diagram
...
- [Features Tour](features-tour.html) - Interactive diagrams, themes, logging
